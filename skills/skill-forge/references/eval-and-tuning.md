...
2. **基线(baseline)** — 有了用例就**立刻量一次分**,它是后续棘轮的 `best` 起点。**没有基线,任何修改都判不了更好 / 更差。** 分两档:
   - **触发基线立刻做**:description 一诞生就能测、便宜,第一时间抓描述缺陷(见主流程 care-scout 例:一测就抓出"找某类医生"漏触发)。
...
- **隐私红线**:隐私敏感技能(如 care-scout 会带出真实医疗从业者姓名 / 评分)**绝不把原始产出提交进公开库**;要留就留**脱敏**版或只留分数。这也是它能力测试宁可"跳过"也不造假的原因。
