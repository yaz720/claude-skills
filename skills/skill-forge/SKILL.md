...
- **命名(kebab-case)**:技能名一律**全小写**,**多个词之间用连字符 `-` 连**(如 `care-scout`、`figure-forge`、`skill-forge`)。
...
- **触发基线立刻做**——description 一诞生就能测、又便宜,第一时间抓描述缺陷(care-scout 就是这样抓到"找某类医生"漏触发的)。
...
- **逐次产出** `evals/capability/outputs/` —— 可重生、可能含隐私(如真实医生数据),**gitignore、不入库**;**隐私敏感技能绝不提交原始产出**(这正是 care-scout 能力测试"跳过"而非造假的原因)。
